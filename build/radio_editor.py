...
def resource_path(relative_name: str) -> str:
    """Works in dev + PyInstaller onefile/onedir (Windows/macOS)."""
...
def _candidate_documents_dirs() -> list[Path]:
    """
    Cross-platform Documents discovery:
      - Uses Qt's Documents location
      - Adds common Windows OneDrive/Documents fallback
    """
    cands: list[Path] = []
    ...
    # Deduplicate (case-insensitive on Windows/macOS typical FS)
    seen = set()
    out: list[Path] = []
    for p in cands:
        key = str(p).lower()
...
def find_game_file(game_folder_name: str) -> Optional[Path]:
    """
    Finds:
      <Documents>/<GameFolder>/live_streams.sii
...
def _find_trailing_brace_tail_start(lines: list[str]) -> int:
    # Keep trailing braces at the bottom (and blank lines above them)
    i = len(lines)
    while i > 0:
        t = lines[i - 1].strip()
        if t == "" or t == "}":
...
def _update_stream_data_count_line(lines: list[str], count: int) -> bool:
    # Updates the stream_data count line (line 4 style): stream_data: <count>
    for i, line in enumerate(lines):
...
def write_live_streams(path: str, original_lines, station_line_indexes, stations, strategy: str = "in_place"):
    ...
    if strategy == "new_slot":
        # Append a clean new block before the trailing braces
...
class StationModel(QAbstractTableModel):
    ...
    def dropMimeData(self, data, action, row, column, parent):
        ...
        if not raw:
            return False
        src_rows = [int(x) for x in raw.split(",") if x.strip().isdigit()]
        if not src_rows or len(src_rows) != 1:
            return False

        src = src_rows[0]
        if src < 0 or src >= len(self.stations):
            return False
        ...
        item = self.stations.pop(src)
        self.stations.insert(dst, item)
        self.endMoveRows()
...
class OpenPage(QWidget):
    def __init__(self, on_open_callback, settings: QSettings):
        ...
        if platform.system().lower() == "darwin":
            tip = QLabel("Mac tip: Files are usually in ~/Documents/American Truck Simulator/ or ~/Documents/Euro Truck Simulator 2/")
            tip.setStyleSheet("color: #bbbbbb;")
...
class EditorPage(QWidget):
    def __init__(self, on_back_callback):
        ...
        info_text = QLabel("Info: The game accepts MP3 streams only. Always test the URL in-game.")
        info_text.setWordWrap(True)
        info_lay.addWidget(info_text)

        link = QLabel('<a href="https://fmstream.org">Find stations at fmstream.org</a>')
        ...
        self.url_info_badge.setToolTip("MP3 streams only. Test the URL in-game.")
    ...
    def save_station(self):
        ...
        if is_insert:
            ...
            if r < 0 or r >= len(self.model.stations):
                is_insert = True
                self.model.beginInsertRows(QModelIndex(), len(self.model.stations), len(self.model.stations))
                self.model.stations.append(s)
                self.model.endInsertRows()
